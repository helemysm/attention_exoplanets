...
train_loader_real = torch.load(config.dataset.train_data_path)
val_loader_real = torch.load(config.dataset.val_data_path)
test_loader_real = torch.load(config.dataset.test_data_path)
...
d_model = config.modelparam.emb_dim
num_layers = config.modelparam.layers
num_class = 2
api_key_model = 'run'


opt = ScheduledOptim(d_model, optim.Adam(d_model, model.parameters(), betas=(0.9, 0.98), eps=4e-09),
            lr=0.001)

model = model_clf(in_feature, seq_len, n_heads, num_class, num_layers, d_model)
net = Network(model, loss, opt, experiment=Experiment(api_key=api_key_model, project_name="exoplanet-attn-w"))

_,_= net.fit({"train": train_loader, "val": test_loader}, epochs=config.training.epochs, )
